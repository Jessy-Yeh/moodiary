...
interface IProps {
  date: string;
  mood: "super-happy" | "happy" | "neutral" | "sad" | "super-sad";
  text: string;
}

export default function MoodCard({ mood, date, text }: IProps) {
  const SVG_PATHS = {
    "super-happy":
  ...
  return (
    <div className={styles[`mood-card`]}>
      <div className={styles.dateAndEmoji}>
        <p>{date}</p>
        <svg
          className={`${styles.emoji} ${styles[`${mood}-emoji`]}`}
          viewBox="0 0 24 24"
        >
          <path d={SVG_PATHS[mood]} />
        </svg>
      </div>
      <p>{text}</p>
    </div>
