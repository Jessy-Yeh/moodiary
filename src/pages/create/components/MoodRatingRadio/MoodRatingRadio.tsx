...
import styles from "./MoodRatingRadio.module.css";
...
interface IProps {
  value: "super-happy" | "happy" | "neutral" | "sad" | "super-sad";
  moodStatus: string;
  setMoodStatus: (moodStatus: string) => void;
}

const MoodRatingRadio = ({ value, moodStatus, setMoodStatus }: IProps) => {
  return (
    <label className={styles.label} htmlFor={value}>
      <input
        type="radio"
        name="rating"
        className={`${styles[value]} ${styles.emojiInput}`}
        onChange={(e) => setMoodStatus(e.target.value)}
        checked={moodStatus === value}
        id={value}
        value={value}
      />
      <svg
        className={`${styles.emoji} ${styles[`${value}-emoji`]}`}
        viewBox="0 0 24 24"
      >
        <path d={SVG_PATHS[value]} />
      </svg>
    </label>
  );
};

export default MoodRatingRadio;
